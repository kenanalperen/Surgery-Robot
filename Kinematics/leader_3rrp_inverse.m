...
% Rotation Matrix
phi=-pi/3; %If added, probably should be minus signed of forward kinematics phi
R_z=[cos(phi) -sin(phi);sin(phi) cos(phi)];
A= R_z*[-x1;-y1]; % Be aware that minuses in forward kin is reflected here, not below!
x=A(1);
y=A(2);
theta=-theta1;
...
% At this point, q_4<-pi is not expected I guess
